...
export default function DataTable({
    ...
    // Handle actions menu
    const toggleMenu = (event: React.MouseEvent, rowId: number) => {
    ...
    const renderCellContent = (column: TableColumn, row: any) => {
        ...
        if (statusConfig[value] || statusConfig[column.key]) {
            return renderStatusBadge(value, column.key);
        }
    ...
    return (
        <div className={`bg-white rounded-lg shadow overflow-hidden ${className}`}>
            <div className={`overflow-x-auto ${maxHeight} overflow-y-auto`}>
                <Table>
                    <TableHeader className={`border-b border-gray-100 dark:border-white/[0.05] ${stickyHeader ? 'sticky top-0 z-10 bg-gray-50' : ''}`}>
                        <TableRow>
                            {selectable && (
                                <TableCell
                                    isHeader
                            ...
                            {columns.map((column) => (
                                <TableCell
                                    key={column.key}
                                    isHeader
                                    className={`px-5 py-3 font-medium text-gray-500 text-${column.align || 'start'} text-theme-xs dark:text-gray-400 ${column.width ? `w-${column.width}` : ''}`}
                                >
                                    {column.label}
                                </TableCell>
                            ))}
                            ...
                            return (
                                ...
                                >
                                    ...
                                    {columns.map((column) => (
                                        <TableCell
                                            key={column.key}
                                            className={`px-4 py-3 text-gray-500 text-${column.align || 'start'} text-theme-sm dark:text-gray-400`}
                                        >
                                    ...
                                    {showActions && actions.length > 0 && (
                                        <TableCell className="px-4 py-3 text-center">
                                            <button
                                                onClick={(e) => toggleMenu(e, row.id || index)}
                ...
                >
                    {actions.map((action, index) => (
                        <button
                            ...
                            className={`w-full px-4 py-2 text-left text-sm hover:bg-gray-100 flex items-center gap-2 ${action.variant === 'danger' ? 'text-red-600' : 'text-gray-700'
                                } ${action.className || ''}`}
                        >
                            {action.icon}
                            {action.label}
