  ...
  return (
    <div className="relative inline-block text-left" ref={dropdownRef}>
      ...
      >
        {selected}
        <FaAngleDown
          className={`ml-auto w-4 h-4 text-gray-500 transition-transform duration-200 ${open ? "rotate-180" : ""
            }`}
        />
      </button>
      ...
      {open && (
        <div className="absolute mt-1 md:w-40 w-full rounded-lg bg-white shadow-lg border border-gray-200 z-50">
          {options.map((opt) => (
            <button
              key={opt}
              className={`w-full flex justify-between items-center text-left px-4 py-2 hover:bg-gray-100 ${selected === opt ? "bg-gray-200 font-semibold" : ""
                }`}
              onClick={() => {
                setSelected(opt);
                setOpen(false);
